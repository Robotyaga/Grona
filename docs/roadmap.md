...
A donor model or local LM Studio model could suggest labels, summaries, seeds, examples, or benchmark answers. Those outputs should be validated, reviewed, optionally assigned to candidate clusters, measured by BenchmarkSuite, and evaluated by GrowthEngine before they become durable knowledge or training data.
